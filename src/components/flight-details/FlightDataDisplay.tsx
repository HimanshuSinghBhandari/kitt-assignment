...
import { flights } from '@/utils/airportData';
...
const FlightDataDisplay: React.FC<FlightDataDisplayProps> = ({
  ...
  const flightOptions = flights
    .filter(f => (f.from === '*' || f.from === fromAirport) && (f.to === '*' || f.to === toAirport))
    ...
    <div key={index} className="flex flex-col">
      ...
      <div className="flex">
        <div className="flex w-4/5">
          <Image src={flight.image} alt={flight.airline} width={40} height={40} className="mr-2" />
          <div className='ml-2'>
            <p className="text-[11px] text-gray-500">{flight.airline} {flight.flightNumber}</p>
            <p className="text-md text-black">{flight.departureTime} - {flight.arrivalTime}</p>
          </div>
          <div className="ml-[8rem] hidden sm:block">
            <p className="text-[11px] text-gray-500">{fromAirport}-{toAirport}</p>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <p className="text-left mb-4">{flightOptions.length} results out of 756</p>
      {flightOptions.length > 0 ? (
        flightOptions.reduce((acc: JSX.Element[], flight, index) => {
          if (index % 2 === 0) {
            acc.push(
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 mb-4 flex flex-col sm:flex-row">
                <div className="w-full sm:w-4/5 space-y-6">
                  {renderFlightCard(flight, index)}
                  {flightOptions[index + 1] && renderFlightCard(flightOptions[index + 1], index + 1)}
                </div>
                <div className="w-full sm:w-1/5 border-t sm:border-t-0 sm:border-l border-gray-200 pt-4 sm:pt-0 sm:pl-4 mt-4 sm:mt-0 flex flex-row sm:flex-col justify-between sm:justify-start">
                  ...
                  <button
                    ...
                    onClick={() => handleSelectFlight(flight, flightOptions[index + 1])}
                  >
                    Select
                  </button>
                </div>
        ...
        <p className="text-center text-gray-600">No flights found for the selected route.</p>
      )}
