...
const FlightSearchResults: React.FC<FlightSearchResultsProps> = ({ 
  ...
  const handleSearch = () => {
    ...
    startSearch();
    // You can add logic here to fetch new flight data based on the updated search parameters
  };
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen bg-white p-4 w-full">
      {!showSearchBar ? (
        <div className="w-full max-w-md bg-white border border-gray-200 rounded-3xl p-2">
          <div className="flex flex-row items-center justify-between space-x-4 ml-2">
            ...
            <div className="flex items-center text-xs  text-gray-600 mr-4">
              <span>{formatDate(searchParams.departureDate)} - {formatDate(searchParams.returnDate)}</span>
              <button onClick={toggleSearchBar} className="ml-4 h-4 w-4 mt-1">
                <FaSearch />
              </button>
            </div>
          </div>
        ...
        <SearchBar
          ...
          onFromAirportChange={(value) => setSearchParams(prev => ({ ...prev, fromAirport: value }))}
          onToAirportChange={(value) => setSearchParams(prev => ({ ...prev, toAirport: value }))}
          onDepartureDateChange={(value) => setSearchParams(prev => ({ ...prev, departureDate: value }))}
          onReturnDateChange={(value) => setSearchParams(prev => ({ ...prev, returnDate: value }))}
      ...
      <div className="w-full mb-4">
        <div className="flex mb-1 items-center justify-between">
          ...
          <span className="text-xs font-semibold text-[#003E39]">{searchState.progress}%</span>
        </div>
          ...
          <div className="w-70 h-70 mt-[4rem] bg-white border border-gray-200 rounded-lg p-6 shadow-lg flex flex-col items-center justify-center z-10">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center mb-1">
                ...
                <p className="text-lg font-semibold text-gray-400">Searching 400+ flights</p>
              </div>
              <p className="text-sm text-gray-600 mb-1">Attaching company rules</p>
